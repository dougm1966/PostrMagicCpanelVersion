    ...
    try {
        ...
        if ($action === 'create' || $action === 'update') {
            ...
            if (empty($slug)) {
                ...
                $original_slug = strtolower(trim(preg_replace('/[^A-Za-z0-9-]+/', '-', $title)));
                $slug = $original_slug;
                
                // If this is an update, get the current page ID
                $current_id = ($action === 'update' && !empty($_POST['id'])) ? intval($_POST['id']) : null;
                
                // Function to check if slug exists
                $slugExists = function($slug_to_check) use ($pdo, $current_id) {
                    $sql = "SELECT COUNT(*) FROM static_pages WHERE slug = ?";
                    $params = [$slug_to_check];
                ...
                // If slug is empty after processing, set a default
                if (empty($slug)) {
                    $slug = 'page';
                }
                ...
                while ($slugExists($slug) && $counter < 100) {
                    $slug = $base_slug . '-' . $counter++;
                }
                
                // If we still have a conflict, append timestamp
                if ($slugExists($slug)) {
                    $slug = $base_slug . '-' . time();
                }
            } else {
                // If slug is provided manually, clean it up
                $slug = strtolower(trim(preg_replace('/[^A-Za-z0-9-]+/', '-', $slug)));
                
                // Check if the manually entered slug already exists
                $current_id = ($action === 'update' && !empty($_POST['id'])) ? intval($_POST['id']) : null;
                $sql = "SELECT COUNT(*) FROM static_pages WHERE slug = ?";
                $params = [$slug];
                ...
                if ($stmt->fetchColumn() > 0) {
                    throw new Exception('The URL slug is already in use. Please choose a different one.');
                }
            }
